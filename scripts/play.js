...
function moveShip() {
  ...
  if (keyboard[39]) {
    ...
    if (ship.y != limit1) {
      ship.y -= 10;8
    } else if(ship.y == limit1) 
    // alert("Has llegado al limite de altura!" + document.location.reload());
    console.log("Has llegado al límite de altura!" + limit1);
  }
  if (keyboard[37]) {
    //DOWN
    if (ship.y != limit2) {
      ship.y += 10;
    } else if (ship.y == limit2) 
    // alert("Has llegado al limite mas bajo de altura!" + document.location.reload());
    console.log("Has llegado al límite de altura!" + limit2);
  }
}
...
function win(){
  if(hud.counter_p == 500)
  {
    alert('Felicidades has ganado ' + document.location.reload());
  }
}
